...
public class Cliente {
	...
	private final static String MSJ_SALDO_INVALIDO = "Saldo Invalido";
	...
	private void setSaldo(double saldoInicial) {
		if (saldoInicial < 1) {
			throw new IllegalArgumentException(MSJ_SALDO_INVALIDO);
		}
	...
	public void procesarOrden(Orden orden) {
		double valor = orden.obtenerPrecio();
		TipoHistorial tipo = null;
		
		if (orden.isCompra()) {
			double calculo= this.saldo - valor;
			if (calculo < 0) {
		...
		this.agregarEntradaEnHistorial(orden.obtenerDatosParaHistorial(), tipo);
